...
type Wallet struct {
	...
	deriveChain     goethereum.ChainStateReader // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *Wallet) SelfDerive(bases []accounts.DerivationPath, chain goethereum.ChainStateReader) {
	w.lock.Lock()
	defer w.lock.Unlock()
