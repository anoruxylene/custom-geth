...
var (
	_ = goeth.ChainReader(&Client{})
	_ = goeth.TransactionReader(&Client{})
	_ = goeth.ChainStateReader(&Client{})
	_ = goeth.ChainSyncReader(&Client{})
	_ = goeth.ContractCaller(&Client{})
	_ = goeth.GasEstimator(&Client{})
	_ = goeth.GasPricer(&Client{})
	_ = goeth.LogFilterer(&Client{})
	_ = goeth.PendingStateReader(&Client{})
	// _ = ethereum.PendingStateEventer(&Client{})
	_ = goeth.PendingContractCaller(&Client{})
)
...
func testTransactionInBlockInterrupted(t *testing.T, client *rpc.Client) {
	...
	if err == nil || err == goeth.NotFound {
		t.Fatal("error should not be nil/notfound")
	}

	// Test tx in block not found.
	if _, err := ec.TransactionInBlock(context.Background(), block.Hash(), 20); err != goeth.NotFound {
